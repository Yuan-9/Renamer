...
const initialProgress = { stage: "idle", current: 0, total: 0 };
...
function App() {
  ...
  useEffect(() => {
    ...
    return renamerApi.onTaskProgress((payload) => setProgress(payload));
  }, []);

  useEffect(() => {
    const timer = setTimeout(() => {
      renamerApi.saveSettings(settings).catch(() => {});
      if (items.length > 0) {
        renamerApi
          .buildPreview({ items, settings })
          .then((response) => setItems(unwrap(response).items))
          .catch((error) => setMessage(error.message));
      }
    }, 180);
  ...
  async function runScan(directory = inputDirectory) {
    ...
    setLastResult(null);
    try {
      const data = unwrap(await renamerApi.scanDirectory({ directory, settings }));
      setItems(data.items);
  ...
  async function executeRename() {
    ...
    setMessage("");
    try {
      const result = unwrap(await renamerApi.executeRename({ items, settings }));
    ...
    <div className="app-shell" onDragOver={(event) => event.preventDefault()} onDrop={onDrop}>
      ...
      <footer className="bottombar">
        ...
        <div className="footer-actions">
          <button type="button" onClick={exportLog} disabled={busy} title="导出日志">
            <Save size={17} />
            日志
          </button>
          {busy ? (
